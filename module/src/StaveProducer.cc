...
StaveProducer::StaveProducer(const std::string & name, const std::string & runcontrol)
...
void StaveProducer::setup_mosaic(std::string conf_dir){
  ...
  std::string mosaic_conf_name = "Mosaic_" + std::to_string(this->stave_id) + ".conf";
  ...
  if(!fb.open(conf_dir, std::ios::in)){
    EUDAQ_THROW("Error when trying to open configuration file for MOSAIC_" + std::to_string(this->stave_id) + ". Aborting producer.");
    return;
  }
  std::istream file(&fb);
...
void StaveProducer::setup_chip_conf(eudaq::Configuration c_conf, int chip_id){
...
scan_info StaveProducer::setup_chip_scans(eudaq::Configuration c_conf, int chip_id){
  scan_info scan_inf;
...
void StaveProducer::setup_chips(std::string conf_dir){
  ...
  for(int i = 0; i < 9; i++){
    ...
    if(!fb.open(conf_dir + chip_conf_name, std::ios::in)){
      EUDAQ_THROW("Error when trying to open configuration file for chip " + std::to_string(i) + ".");
    }
    std::istream file(&fb);
...
void StaveProducer::RunLoop(){
  // initialize counters
  int empty_frames = 0, non_empty_frames = 0, trg_n = 0;
  ...
  while(running){
    ...
    for(int i = 0; i < chips.size(); i++){
      ...
      if(res <= 0 || data_size_dummy <= 66) continue;
    ...
    ev->SetTag("Stave ID", this->stave_id);
  ...
  EUDAQ_INFO("Stave:" + std::to_string(this->stave_id) + " Empty Frames:"     + std::to_string(empty_frames));
