...
TConfig::TConfig(int nBoards, std::vector<int> chipIds, TBoardType boardType, TDeviceType dev_type)
{
...
TChipConfig *TConfig::GetChipConfigById(int chipId)
{
  for (unsigned int i = 0; i < fChipConfigs.size(); i++) {
    if (fChipConfigs.at(i)->GetChipId() == chipId) return fChipConfigs.at(i);
  }
...
THicConfig *TConfig::GetHicConfigById(int modId)
{
  for (unsigned int i = 0; i < fHicConfigs.size(); i++) {
    if (fHicConfigs.at(i)->GetModId() == modId) return fHicConfigs.at(i);
  }
  // throw exception here.
