...
void AlpideConfig::WritePixRegSingle(
    TAlpide *chip, Alpide::TPixReg reg, bool data, int row, int col) {
        WritePixConfReg(chip, reg, data);
...
void AlpideConfig::ApplyStandardDACSettings(TAlpide *chip, float backBias) {
    if (backBias == 0) {
        ...
        std::cout << "Settings not defined for back bias " 
            << backBias << " V. Please set manually."
            << std::endl;
    }
}

// should it be allowed to pass a config or should always the chip config be used?
void AlpideConfig::ConfigureFromu(
    TAlpide *chip, Alpide::TPulseType pulseType, 
    bool testStrobe, TChipConfig *config) {
...
int AlpideConfig::ConfigureMaskStage(
    TAlpide *chip, int nPix, int iStage, bool Mask, bool Select) {
        if (iStage == 0) {
            EnableDoubleColumns(chip);
        }
        // check that nPix is one of (1, 2, 4, 8, 16, 32)
        if ((nPix <= 0) || (nPix & (nPix - 1)) || (nPix > 32)) {
            std::cout << "Warning: bad number of pixels for mask stage (" 
                << nPix << ", using 1 instead"
                << std::endl;
            nPix = 1;
        }
        if (Mask) {
...
void AlpideConfig::WriteControlReg(
    TAlpide *chip, Alpide::TChipMode chipMode, TChipConfig *config) {
        if (!config) {
            config = chip->GetConfig();
        }
