...
I2CSysPll::pllRegisters_t 
TReadoutBoardMOSAIC::sysPLLregContent(new uint16_t[22]{
    /* Register 0: */ 0x02A9,
    /* Register 1: */ 0x0000,
    /* Register 2: */ 0x000E,
    /* Register 3: */ 0x08F5,
...
TReadoutBoardMOSAIC::TReadoutBoardMOSAIC(
    TConfig *config, TBoardConfigMOSAIC *boardConfig)
    : TReadoutBoard(boardConfig), fBoardConfig(boardConfig), 
    i2cBus(0x0), i2cBusAux(0x0), pb(0x0),
    pulser(0x0), dr(0x0), 
    theVersionMaj(-1), theVersionMin(-1), 
    trgRecorder(0x0), trgDataParser(0x0), coordinator(0x0), 
    readTriggerInfo(false) {
        init();
}
...
int TReadoutBoardMOSAIC::WriteChipRegister(
    uint16_t address, uint16_t value, TAlpide *chipPtr) {
...
int TReadoutBoardMOSAIC::ReadChipRegister(
    uint16_t address, uint16_t &value, TAlpide *chipPtr) {
        uint_fast16_t Cii    = chipPtr->GetConfig()->GetParamValue("CONTROLINTERFACE");
        uint8_t       chipId = chipPtr->GetConfig()->GetChipId();
...
int TReadoutBoardMOSAIC::SetTriggerConfig(
    bool enablePulse, bool enableTrigger, 
...
void TReadoutBoardMOSAIC::enableControlInterfaces(bool en) {
    for (int Cii = 0; Cii < MAX_MOSAICCTRLINT; Cii++) {
        ...
        controlInterface[Cii]->addDisableME(
            fBoardConfig->GetManchesterDisable() == 1 ? true : false);
        controlInterface[Cii]->execute();
    }
}
...
void TReadoutBoardMOSAIC::enableControlInterface(int interface, bool en) {
    if (interface < MAX_MOSAICCTRLINT) {
        ...
        controlInterface[interface]->addDisableME(
            fBoardConfig->GetManchesterDisable() == 1 ? true : false);
 ...
 */
void TReadoutBoardMOSAIC::WriteTransceiverDRP(
    size_t Aindex, uint16_t address, uint16_t val, bool execute) {
...
/*
  ...
  parameters: Aindex - Transceiver id
              ...
              execute - execute transaction
*/
void TReadoutBoardMOSAIC::WriteTransceiverDRPField(
    size_t Aindex, uint16_t address, uint16_t size,
...
/*
   ...
   parameters: Aindex  - Transceiver id
               ...
               execute - execute transaction
*/
void TReadoutBoardMOSAIC::ReadTransceiverDRP(
    size_t Aindex, uint16_t address, uint32_t *value, bool execute) {
    if (Aindex >= MAX_MOSAICTRANRECV) {
        std::cerr << "Invalid Transceiver index " << Aindex << "\n";
        return;
    }
