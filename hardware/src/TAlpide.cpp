...
void TAlpide::SetEnableWithBB(bool Enable) { 
    fConfig->SetEnableWithBB(Enable); 
}
...
int TAlpide::ModifyRegisterBits(
    TRegister address, uint8_t lowBit, 
    uint8_t nBits, uint16_t value,
    bool verify) {
        return ModifyRegisterBits((uint16_t)address, lowBit, nBits, value, verify);
}

int TAlpide::ModifyRegisterBits(
    uint16_t address, uint8_t lowBit, 
    uint8_t nBits, uint16_t value,
    bool verify) {
 ...
 *
 */
uint16_t TAlpide::SetTheADCCtrlRegister(
    Alpide::TADCMode Mode, Alpide::TADCInput SelectInput,
...
int TAlpide::CalibrateADC() {
    ...
    fADCHalfLSB = false;
    fADCSign    = false;
    SetTheADCCtrlRegister(
        Alpide::MODE_CALIBRATE, Alpide::INP_AVSS, Alpide::COMP_296uA,
        Alpide::RAMP_1us);
...
float TAlpide::ReadTemperature() {
    ...
    usleep(5000);
    SetTheADCCtrlRegister(
        Alpide::MODE_MANUAL, Alpide::INP_Temperature, Alpide::COMP_296uA,
        Alpide::RAMP_1us);
    fReadoutBoard->SendCommand(Alpide::COMMAND_ADCMEASURE, this);
...
float TAlpide::ReadDACVoltage(Alpide::TRegister ADac) {
    ...
    usleep(5000);
    SetTheADCCtrlRegister(
        Alpide::MODE_MANUAL, Alpide::INP_DACMONV, Alpide::COMP_296uA,
        Alpide::RAMP_1us);
    fReadoutBoard->SendCommand(Alpide::COMMAND_ADCMEASURE, this);
    usleep(5000); // Wait for the measurement > of 5 milli sec
...
float TAlpide::ReadDACCurrent(Alpide::TRegister ADac) {
    ...
    usleep(5000);
    SetTheADCCtrlRegister(
        Alpide::MODE_MANUAL, Alpide::INP_DACMONI, Alpide::COMP_296uA,
        Alpide::RAMP_1us);
